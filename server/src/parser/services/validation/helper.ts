...
(async () => {
    ...
    if (process.send) {
		...
		const getDocumentPromisePromise = new Promise(resolve => { getDocumentPromisePromiseResolver = resolve; });

        let solidityCompileConfirmationPromiseResolver: any;
		const solidityCompileConfirmationPromise = new Promise(resolve => { solidityCompileConfirmationPromiseResolver = resolve; });
