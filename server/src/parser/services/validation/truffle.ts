...
export async function truffleValidator(uri: string, document: TextDocument): Promise<{ [uri: string]: Diagnostic[] }> {
    try {
        ...
        if (process.send) {
            ...
            try {
                ...
                if (err && err.name === "CompileError") {
                    ...
                    const relativePath = splittedLocation[1].charAt(0) === "/" ? splittedLocation[1].slice(1) : splittedLocation[1];
                    ...
                    return {
                        [relativePath]: [<Diagnostic>{
                            ...
                            severity: message.split(":")[0].toLowerCase().includes("error") ? DiagnosticSeverity.Error : DiagnosticSeverity.Warning,
                            message: message,
                            range: Range.create({
                                line: +startLine,
                                ...
                                line: +startLine,
                                character: +startCharacter + (splittedMessage[2].trim().length)
                            })
                        }]
                    };
                }
