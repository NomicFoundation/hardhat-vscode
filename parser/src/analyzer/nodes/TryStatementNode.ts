...
export class TryStatementNode implements Node {
    ...
    accept(find: FinderType, documentsAnalyzerTree: DocumentsAnalyzerTree, orphanNodes: Node[], parent?: Node, expression?: Node): Node {
        ...
        for (const returnParameter of this.astNode.returnParameters || []) {
            find(returnParameter, this.uri).accept(find, documentsAnalyzerTree, orphanNodes, this);
        }
        ...
        for (const catchClause of this.astNode.catchClauses || []) {
            find(catchClause, this.uri).accept(find, documentsAnalyzerTree, orphanNodes, this);
        }
