...
export class IndexRangeAccessNode extends Node {
    ...
    constructor (indexRangeAccess: IndexRangeAccess, uri: string, rootPath: string) {
        super(indexRangeAccess, uri, rootPath);
    ...
    accept(find: FinderType, orphanNodes: Node[], parent?: Node, expression?: Node): Node {
        ...
        if (this.astNode.indexStart) {
            find(this.astNode.indexStart, this.uri, this.rootPath).accept(find, orphanNodes, parent);
        }

        if (this.astNode.indexEnd) {
            find(this.astNode.indexEnd, this.uri, this.rootPath).accept(find, orphanNodes, parent);
        }
