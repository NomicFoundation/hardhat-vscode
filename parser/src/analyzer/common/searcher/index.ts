...
import {
    ...
    FunctionDefinitionNode, VariableDeclarationNode, Searcher as ISearcher
} from "@common/types";
...
export class Searcher implements ISearcher {
     ...
     */
    public findParent(node: Node, from?: Node, searchInInheritanceNodes = false): Node | undefined {
     ...
     */
    public findDefinitionNodes(uri: string, position: Position, from: Node): Node[] {
     ...
     */
    public checkIsNodeVisible(uri: string, position: Position, node: Node): boolean {
     ...
     */
    private search(node: Node, from?: Node | undefined, searchInInheritanceNodes?: boolean, visitedNodes?: Node[], visitedFiles?: string[]): Node | undefined {
        if (!visitedNodes) {
            visitedNodes = [];
        }
     ...
     * @returns Node that can connect to the forwarded node.
     */
    private searchInImportNodes(visitedFiles: string[], node: Node, from?: Node | undefined): Node | undefined {
        if (!from) {
            return undefined;
        }
     ...
     */
    private searchInExpressionNode(uri: string, position: Position, expressionNode?: Node | undefined): Node | undefined {
        if (!expressionNode) {
            return undefined;
        }
     ...
     * @returns Wanted {@link Node} if exist.
     */
    private walk(uri: string, position: Position, from?: Node, searchInExpression = false, visitedNodes?: Node[], visitedFiles?: string[]): Node | undefined {
        if (!visitedNodes) {
            visitedNodes = [];
        }
     ...
     * @param isShadowedByParent Is current from node shadowed by position.
     */
    private _findDefinitionNodes(uri: string, position: Position, from: Node | undefined, definitionNodes: Node[], isShadowedByParent = false, visitedNodes?: Node[], visitedFiles?: string[]): void {
        if (!visitedNodes) {
            visitedNodes = [];
        }
