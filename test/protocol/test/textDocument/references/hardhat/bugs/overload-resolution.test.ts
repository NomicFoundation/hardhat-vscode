...
describe('[hardhat] references bug - overload resolution', () => {
  ...
  before(async () => {
    ...
    refsPath = getProjectPath('hardhat/contracts/references/bugs/overload-resolution/FunctionRefs.sol')
  ...
  test.skip('console.log(address, bool), from its declaration', async () => {
    const locations = await client.findReferences(toUri(consolePath), makePosition(243, 10))

    expect(sorted(locations)).to.deep.equal(
      sorted([
        { uri: toUri(consolePath), range: makeRange(243, 10, 243, 13) },
