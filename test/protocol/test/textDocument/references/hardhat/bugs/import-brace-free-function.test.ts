...
describe('[hardhat] references bug - free function imported by name in braces is never bound', () => {
  ...
  before(async () => {
    ...
    flDefsPath = getProjectPath('hardhat/contracts/references/file-level/FlRefDefs.sol')
    flUserPath = getProjectPath('hardhat/contracts/references/file-level/FlRefUser.sol')
