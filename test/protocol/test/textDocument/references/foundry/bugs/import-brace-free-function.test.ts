...
describe('[foundry] references bug - free function imported by name in braces is never bound', () => {
  ...
  before(async () => {
    ...
    flDefsPath = getProjectPath('foundry/src/references/file-level/FlRefDefs.sol')
    flUserPath = getProjectPath('foundry/src/references/file-level/FlRefUser.sol')
