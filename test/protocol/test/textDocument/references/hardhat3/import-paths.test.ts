...
describe('[hardhat3] references - import-paths', () => {
  let basePath: string
  ...
  before(async () => {
    ...
    ownableUserPath = getProjectPath('hardhat3/contracts/references/import-paths/IpOwnable.sol')
    consoleUserPath = getProjectPath('hardhat3/contracts/references/import-paths/IpConsole.sol')
    ozPath = getProjectPath('hardhat3/node_modules/@openzeppelin/contracts/access/Ownable.sol')
    contextPath = getProjectPath('hardhat3/node_modules/@openzeppelin/contracts/utils/Context.sol')
  ...
  test('contract reached through a remapping and a relative path, from its declaration', async () => {
    const locations = await client.findReferences(toUri(remappedPath), makePosition(3, 9))

    expect(sorted(locations)).to.deep.equal(
      sorted([
        { uri: toUri(remappedPath), range: makeRange(3, 9, 3, 19) },
        { uri: toUri(relPath), range: makeRange(6, 4, 6, 14) },
        { uri: toUri(remapPath), range: makeRange(6, 4, 6, 14) },
      ])
    )
  })

  test('contract reached through a remapping and a relative path, from the remapped use', async () => {
    const locations = await client.findReferences(toUri(remapPath), makePosition(6, 4))
  ...
  test('contract reached through a remapping and a relative path, from the relative use', async () => {
    const locations = await client.findReferences(toUri(relPath), makePosition(6, 4))
  ...
  test('console library function, from its declaration', async () => {
    const locations = await client.findReferences(toUri(consolePath), makePosition(87, 13))

    expect(sorted(locations)).to.deep.equal(
      sorted([
        { uri: toUri(consolePath), range: makeRange(87, 13, 87, 22) },
