...
describe('[hardhat3] documentSymbol bug - Slang rejects transient as a name from 0.8.27, which solc still accepts', () => {
  let transientNamePath: string

  before(async () => {
    client = await getInitializedClient()
    transientNamePath = getProjectPath(
      'hardhat3/contracts/document-symbols/bugs/transient-identifier/TransientName.sol'
    )
