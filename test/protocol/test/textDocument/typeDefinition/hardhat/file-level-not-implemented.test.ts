...
describe('[hardhat] typeDefinition - file-level (not implemented)', () => {
  let defsPath: string
  let userPath: string

  before(async () => {
    client = await getInitializedClient()
    defsPath = getProjectPath('hardhat/contracts/type-definition/file-level/FlDefs.sol')
    userPath = getProjectPath('hardhat/contracts/type-definition/file-level/FlUser.sol')
