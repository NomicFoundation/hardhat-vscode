...
describe('[hardhat3] typeDefinition bug - a contract name in a new expression resolves to its constructor', () => {
  let derivedPath: string
  let basePath: string

  before(async () => {
    client = await getInitializedClient()
    derivedPath = getProjectPath('hardhat3/contracts/type-definition/inheritance/IhDerived.sol')
    basePath = getProjectPath('hardhat3/contracts/type-definition/inheritance/IhBase.sol')
