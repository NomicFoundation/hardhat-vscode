...
describe('[hardhat3] typeDefinition bug - bare call binds to the first-declared overload', () => {
  let overloadsPath: string

  before(async () => {
    client = await getInitializedClient()
    overloadsPath = getProjectPath('hardhat3/contracts/type-definition/bugs/overload-resolution/FnOverloads.sol')
