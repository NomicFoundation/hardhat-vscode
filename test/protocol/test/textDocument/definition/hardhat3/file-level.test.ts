...
describe('[hardhat3] definition - file-level', () => {
  let defsPath: string
  let userPath: string
  ...
  test('file-level event in emit, from another file', async () => {
    const location = await client.findDefinition(toUri(userPath), makePosition(14, 15))

    expect(location).to.deep.equal({
      uri: toUri(defsPath),
      range: makeRange(17, 6, 17, 11),
    })
  })

  test('library in a file-level using directive', async () => {
    const location = await client.findDefinition(toUri(userPath), makePosition(6, 8))

    expect(location).to.deep.equal({
      uri: toUri(defsPath),
      range: makeRange(35, 8, 35, 16),
    })
  })
})
