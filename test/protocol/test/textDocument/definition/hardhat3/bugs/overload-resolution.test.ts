...
describe('[hardhat3] definition bug - overload resolution', () => {
  ...
  before(async () => {
    ...
    sameArityPath = getProjectPath('hardhat3/contracts/definition/bugs/overload-resolution/SameArity.sol')
    callerPath = getProjectPath('hardhat3/contracts/definition/bugs/overload-resolution/ConsoleCaller.sol')
    consolePath = getProjectPath('hardhat3/contracts/definition/../../node_modules/hardhat/console.sol')
  ...
  // Bug: returns log(uint256 p0) (191:13-191:16), the first one-argument log.
  test.skip('console.log with a string goes to log(string)', async () => {
    ...
    expect(location).to.deep.equal({
      ...
      range: makeRange(195, 13, 195, 16),
    })
  })

  // Bug: returns log(uint256 p0, uint256 p1) (207:13-207:16), the first two-argument log.
