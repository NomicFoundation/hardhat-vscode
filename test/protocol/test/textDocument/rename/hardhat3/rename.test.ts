...
describe('[hardhat3] rename', () => {
  ...
  before(async () => {
    ...
    testPath = getProjectPath('hardhat3/contracts/rename/Test.sol')
    multiImportPath = getProjectPath('hardhat3/contracts/rename/MultiImport.sol')
  ...
  test('[multi-file][member access] - rename', async function () {
    ...
    expect(workspaceEdit).to.deep.equal({
      changes: {
        [toUri(getProjectPath('hardhat3/contracts/rename/Foo.sol'))]: [
          {
